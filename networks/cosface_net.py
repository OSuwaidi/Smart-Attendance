...
if __name__ == "__main__":
    # Example usage
    model = Sphere(type=20, is_gray=False)
    input_tensor = torch.randn(4, 3, 112, 112)
    output = model(input_tensor)
    print(f"Output shape: {output.shape}")

    model_sphere = Sphere(type=64, is_gray=True)
    input_tensor_sphere = torch.randn(4, 1, 112, 112)
    output_sphere = model_sphere(input_tensor_sphere)
    print(f"Output shape for sphere64: {output_sphere.shape}")

    model_ir = LResNet50E_IR(is_gray=False)
    input_tensor_ir = torch.randn(4, 3, 112, 112)
    output_ir = model_ir(input_tensor_ir)
    print(f"Output shape for LResNet50E_IR: {output_ir.shape}") 
